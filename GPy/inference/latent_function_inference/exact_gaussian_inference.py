...
class ExactGaussianInferenceMulti(LatentFunctionInference):
    ...
    def inference(self,model, kern, X, likelihood, Y, mean_function=None, Y_metadata=None, K=None, variance=None, Z_tilde=None):
        """
        Returns a Posterior class containing essential quantities of the posterior
        """

        low_m, low_var   =   model._raw_predict(X)
        ...
        alpha2,_    =   dpotrs(LW, low_m, lower=1)
        alpha3,_    =   dpotrs(LW, Y, lower=1)

        rho         =   np.sum(alpha3 * low_m) / np.sum(alpha2 * low_m)

        YYT_factor = Y-m-(rho*low_m)
