...
class multiGP(Model):
    ...
    """
    def __init__(self, X, Y, model, kernel, likelihood, mean_function=None, inference_method=None, name='multi-gp', Y_metadata=None, normalizer=False):
        super(multiGP, self).__init__(name)
    ...
    @staticmethod
    def _build_from_input_dict(input_dict, data=None):
        input_dict = multiGP._format_input_dict(input_dict, data)
        return GP(**input_dict)
    ...
    def parameters_changed(self):
        ...
        .. warning::
            ...
            this method yourself, there may be unexpected consequences.
        """
        self.posterior, self._log_marginal_likelihood, self.grad_dict, self.rho = self.inference_method.inference(self.low, self.kern, self.X, self.likelihood, self.Y_normalized, self.mean_function, self.Y_metadata)
        self.likelihood.update_gradients(self.grad_dict['dL_dthetaL'])
        self.kern.update_gradients_full(self.grad_dict['dL_dK'], self.X)
        if self.mean_function is not None:
            self.mean_function.update_gradients(self.grad_dict['dL_dm'], self.X)
    ...
    def _raw_predict(self, Xnew, full_cov=False, kern=None):
        ...
        if self.mean_function is not None:
            mu += self.mean_function.f(Xnew)
        mu_low, var_low =   self.model.predict(Xnew)

        mu  +=  self.rho * mu_low 
        var +=  self.rho**2 * var_low
        return mu, var
